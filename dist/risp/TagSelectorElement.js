...
const TagsSelectorRenderer = (props) => {
    ...
    if ('types' in element) {
        Selector = (react_1.default.createElement(__1.TagGroup, { tags: tags, single: !!element.single, types: element.types, onChange: (selected) => {
        ...
        Selector = (react_1.default.createElement(__1.TagGroup, { tags: tags, single: !!element.single, options: element.options, onChange: (selected) => {
                setSelected(selected);
                const newValue = element.single ? selected[0] : selected;
                element.triggerHandler && element.triggerHandler({ type: 'onChange', name: element.name, value: newValue }, props);
            }, selected: selected }));
